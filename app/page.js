...
export default function Home() {
  ...
  const autoWeekLabel = (() => {
    const d = weekDates[0]; // 월요일
    const m = d.getMonth() + 1;
    // 해당 월 1일부터 이 월요일까지 몇 번째 주인지 계산
    const firstOfMonth = new Date(d.getFullYear(), d.getMonth(), 1);
    const firstDayOfWeek = firstOfMonth.getDay(); // 0=일, 1=월...
    const dayOfMonth = d.getDate();
    // 1일이 포함된 주를 1주차로
    const weekNum = Math.ceil((dayOfMonth + (firstDayOfWeek === 0 ? 6 : firstDayOfWeek - 1)) / 7);
  ...
  const members = [...new Set(
    items.flatMap((item) => (item["진행인원"] || "").split(/[,，]\s*/).filter(Boolean))
  )].join(", ");
  const mailSubject = () => {
